...
const fetchData = async () => {
  try {
    ...
    const volunteerRes = await getVolunteers();
    ...
    const sorted = [...shifts].sort((a, b) => a.date.localeCompare(b.date) || a.time.localeCompare(b.time));
      ...
      cells.push(
        <div key={`d${d}`} className={`bg-surface p-3 min-h-[110px] relative ${isToday ? 'bg-blue-50/50' : ''}`}>
          ...
          <div className="mt-2 space-y-1">
            {dayShifts.slice(0, 3).map((s, index) => {
  const vol = volunteers.find(v => v._id === s.volunteerId);
  const evt = events.find(e => e._id === s.eventId);
  ...
  return (
    <div
      key={s._id}
      className={`${cellColors[index % 3]} px-2 py-1 rounded-md text-[10px] font-medium truncate border cursor-default hover:brightness-95`}
      title={`${vol.firstName} ${vol.lastName} - ${evt.title} @ ${s.time}`}
    >
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8 scrollbar-hide">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-5 mb-8">
          <div className="bg-primary p-6 rounded-2xl text-white shadow-lg shadow-primary/20">
            ...
            <div className="text-[36px] font-bold text-white mb-1" style={{fontFamily:"'Plus Jakarta Sans',sans-serif"}}>{shifts.length}</div>
...
const evt = events.find(e => e._id === s.eventId);
              if (!vol || !evt) return null;
              return (
                <div key={s._id} className="px-6 py-4 flex items-center justify-between hover:bg-surface-container-low transition-colors">
                  <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <button onClick={() => { if (window.confirm('Remove this shift?')) (async () => {
  try {
    await deleteShift(s._id);
    setShifts(shifts.filter(shift => shift._id !== s._id));
  } catch (error) {
    console.log(error);
  }
})(); }} className="w-9 h-9 flex items-center justify-center rounded-xl text-on-surface-variant hover:bg-error/5 hover:text-error transition-all cursor-pointer">
