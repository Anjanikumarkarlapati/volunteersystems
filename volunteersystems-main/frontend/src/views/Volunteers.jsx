...
export default function Volunteers() {
  const { data, deleteVolunteer } = useData();
    ...
    <div className="flex-1 overflow-y-auto p-6 md:p-8 scrollbar-hide">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {filtered.map(v => {
            const ci = v.id % avatarColors.length;
            return (
              <div key={v.id} className="bg-surface-container-lowest rounded-2xl border border-outline-variant p-6 card-hover">
                ...
                <div className="flex gap-2 pt-4 border-t border-outline-variant">
                  ...
                  <button onClick={() => { if (window.confirm('Remove this volunteer?')) deleteVolunteer(v.id); }} className="flex-1 py-2.5 border border-error/20 text-error rounded-xl font-label-md text-[12px] hover:bg-error/5 transition-all cursor-pointer">Remove</button>
                </div>
              </div>
            );
          })}
