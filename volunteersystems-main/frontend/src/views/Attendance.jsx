...
export default function Attendance() {
  const { data, updateAttendance } = useData();
  const [filter, setFilter] = useState('all');

  const activeEvents = data.events.filter(e => e.status !== 'completed');
  let list = data.attendance;
  if (filter !== 'all') list = list.filter(a => a.eventId === parseInt(filter));
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-outline-variant">
              {list.length === 0 ? (
                ...
                const evt = data.events.find(e => e.id === a.eventId);
                if (!vol || !evt) return null;
                const st = statusStyles[a.status];
                const ci = a.id % avatarColors.length;
                return (
                  <tr key={a.id} className="hover:bg-surface-container-low transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <select value={a.status} onChange={e => updateAttendance(a.id, e.target.value)} className="px-3 py-1.5 bg-surface-container-low border border-outline-variant rounded-xl text-[12px] focus:outline-none focus:border-primary">
