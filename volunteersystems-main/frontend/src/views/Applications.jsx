...
export default function Applications() {
  const { data, approveApplication, rejectApplication, deleteApplication } = useData();
          ...
          {filtered.map(a => (
            <div key={a.id} className="bg-surface-container-lowest rounded-2xl border border-outline-variant p-6 card-hover animate-fade-in">
              ...
              <div className="flex gap-2">
                {a.status === 'pending' ? (
                  <>
                    <button onClick={() => approveApplication(a.id)} className="flex-1 py-2.5 bg-gradient-to-r from-emerald-500 to-emerald-600 text-on-secondary rounded-xl font-label-md text-[12px] hover:shadow-lg hover:shadow-emerald-200 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer">
                    ...
                    <button onClick={() => rejectApplication(a.id)} className="flex-1 py-2.5 border border-rose-200 text-rose-600 rounded-xl font-label-md text-[12px] hover:bg-rose-50 transition-all cursor-pointer">
                  ...
                  <button onClick={() => { if (window.confirm('Remove this application?')) deleteApplication(a.id); }} className="flex-1 py-2.5 border border-outline-variant rounded-xl font-label-md text-[12px] text-on-surface-variant hover:bg-surface-variant transition-all cursor-pointer">Remove</button>
                )}
              </div>
            </div>
          ))}
