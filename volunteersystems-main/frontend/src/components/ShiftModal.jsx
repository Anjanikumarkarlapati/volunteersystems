...
export default function ShiftModal({ onClose }) {
  ...
  const activeVols = data.volunteers.filter(v => v.status === 'Active');
  const activeEvents = data.events.filter(e => e.status !== 'completed');
  const [volunteerId, setVolunteerId] = useState(activeVols[0]?.id || '');
  const [eventId, setEventId] = useState(activeEvents[0]?.id || '');
  const [date, setDate] = useState(new Date().toISOString().split('T')[0]);
  const [time, setTime] = useState('09:00');

  const handleSave = () => {
    if (!volunteerId || !eventId || !date) { alert('All fields required'); return; }
    addShift({ volunteerId: parseInt(volunteerId), eventId: parseInt(eventId), date, time });
    onClose();
  };
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <select value={volunteerId} onChange={e => setVolunteerId(e.target.value)} className="w-full px-4 py-2.5 bg-surface-container-low border border-outline-variant rounded-xl text-[14px] transition-all">
              {activeVols.map(v => <option key={v.id} value={v.id}>{v.firstName} {v.lastName}</option>)}
            </select>
          </div>
          <div>
